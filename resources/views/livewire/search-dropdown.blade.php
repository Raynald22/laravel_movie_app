<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    ...
    {{-- <!-- Untuk focus ke search bila menekan tombol '/' --> --}}
    x-ref="search"
    @keydown.window="
    ...
    "
    {{-- <!-- Jika di klik lagi ke search akan memunculkan list lagi --> --}}
    @focus="isOpen = true"
    @keydown="isOpen = true"
                ...
                <ul>
                    @foreach ($searchResults as $result)
                        <li class="border-b border-gray-700">
                            ...
                            {{-- Jika di tab di search, akan menyembunyikan list --}}
                            @if ($loop->last)
                            ...
                            <span class="ml-4">{{ $result['title'] }}</a></span>
                        </li>
                    @endforeach
                </ul>
            @else
                <div class="px-3 py-3 text-red-500">No results found for "{{ $search }}".</div>
            @endif
        </div>
    @endif
</div>
